...
void DrawBillboardRec(Camera camera, Texture2D texture, RaylibRectangle source, Vector3 position, Vector2 size, Color tint)
{
...
void DrawBillboardPro(Camera camera, Texture2D texture, RaylibRectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2 origin, float rotation, Color tint)
{
...
static Image LoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as an uri (base64 or path)
    {
        ...
        {
            image = RaylibLoadImage(TextFormat("%s/%s", texPath, cgltfImage->uri));
        }
    }
