...
*       Load default raylib font on initialization to be used by RaylibDrawText() and MeasureText().
*       If no default font loaded, RaylibDrawTextEx() and MeasureTextEx() are required.
*
...
// NOTE: Default font is loaded on InitWindow() and disposed on RaylibCloseWindow() [module: core]
...
extern void LoadFontDefault(void)
{
    ...
    defaultFont.recs = (RaylibRectangle *)RL_MALLOC(defaultFont.glyphCount * sizeof(RaylibRectangle));
...
#if defined(SUPPORT_FILEFORMAT_TTF)
Image GenImageFontAtlas(const GlyphInfo *chars, RaylibRectangle **charRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
...
#define SUPPORT_COMPRESSED_FONT_ATLAS
    ...
    byteCount += sprintf(txtData + byteCount, "static const RaylibRectangle fontRecs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
...
void DrawFPS(int posX, int posY)
{
    ...
    RaylibDrawText(TextFormat("%2i FPS", GetFPS()), posX, posY, 20, color);
}
...
void RaylibDrawText(const char *text, int posX, int posY, int fontSize, Color color)
{
    ...
    if (GetFontDefault().texture.id != 0)
    {
        ...
        RaylibDrawTextEx(GetFontDefault(), text, position, (float) fontSize, (float) spacing, color);
    }
}

// Draw text using Font
// NOTE: chars spacing is NOT proportional to fontSize
void RaylibDrawTextEx(Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
...
void DrawTextPro(Font font, const char *text, Vector2 position, Vector2 origin, float rotation, float fontSize, float spacing, Color tint)
{
    ...
    RaylibDrawTextEx(font, text, (Vector2) {0.0f, 0.0f}, fontSize, spacing, tint);
...
// NOTE: If codepoint is not found in the font it fallbacks to '?'
RaylibRectangle GetGlyphAtlasRec(Font font, int codepoint)
{
...
static Font LoadBMFont(const char *fileName)
{
    ...
    Image imFont = RaylibLoadImage(imPath);
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        // Get character rectangle in the font atlas texture
        font.recs[i] = (RaylibRectangle){(float)charX, (float)charY, (float)charWidth, (float)charHeight };
