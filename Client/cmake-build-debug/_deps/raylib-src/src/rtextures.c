...
Image RaylibLoadImage(const char *fileName)
{
    Image image = { 0 };
...
Image ImageFromImage(Image image, RaylibRectangle rec)
{
    Image result = { 0 };
...
void ImageCrop(Image *image, RaylibRectangle crop)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                ImageDraw(&imText, font.glyphs[index].image, (RaylibRectangle){0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
...
void ImageAlphaCrop(Image *image, float threshold)
{
    ...
    RaylibRectangle crop = GetImageAlphaBorder(*image, threshold);
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
RaylibRectangle GetImageAlphaBorder(Image image, float threshold)
{
...
{
    ImageDrawRectangleRec(dst, (RaylibRectangle){(float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, RaylibRectangle rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void ImageDrawRectangleLines(Image *dst, RaylibRectangle rec, int thick, Color color)
{
...
void ImageDraw(Image *dst, Image src, RaylibRectangle srcRec, RaylibRectangle dstRec, Color tint)
{
    // Security check to avoid program crash
...
Texture2D LoadTexture(const char *fileName)
{
    ...
    Image image = RaylibLoadImage(fileName);
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, image, faceRecs[i], (RaylibRectangle){0, (float)size * i, (float)size, (float)size }, WHITE);
        }
...
// NOTE: pixels data must match texture.format
void UpdateTextureRec(Texture2D texture, RaylibRectangle rec, const void *pixels)
{
...
void DrawTextureRec(Texture2D texture, RaylibRectangle source, Vector2 position, Color tint)
{
...
void DrawTexturePro(Texture2D texture, RaylibRectangle source, RaylibRectangle dest, Vector2 origin, float rotation, Color tint)
{
...
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, RaylibRectangle dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
